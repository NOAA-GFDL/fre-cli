...
def generate_time_average(infile=None, outfile=None, pkg=None, var=None, unwgt=False, avg_type=None):
    ...
    if pkg == "cdo":
        ...
        myavger = cdoTimeAverager(pkg=pkg, var=var, unwgt=unwgt, avg_type=avg_type)

    elif pkg == "fre-nctools":
        from .frenctoolsTimeAverager import frenctoolsTimeAverager

        myavger = frenctoolsTimeAverager(pkg=pkg, var=var, unwgt=unwgt, avg_type=avg_type)
        ...
        myavger = frepytoolsTimeAverager(pkg=pkg, var=var, unwgt=unwgt, avg_type=avg_type)
...
def generate(inf=None, outf=None, pkg=None, var=None, unwgt=False, avg_type=None):
    """click entrypoint to time averaging routine"""
    exitstatus = generate_time_average(inf, outf, pkg, var, unwgt, avg_type)
    if exitstatus != 0:
