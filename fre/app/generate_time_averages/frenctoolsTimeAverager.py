...
class frenctoolsTimeAverager(timeAverager):
    ...
    def generate_timavg(self, infile=None, outfile=None):
        ...
        if self.avg_type not in ['month','all']:
            log_and_raise(f'avg_type= {self.avg_type} not supported by this class at this time.')
        ...
        if infile is None:
            log_and_raise('Need an input file, specify a value for the infile argument')
        ...
        if shutil.which('timavg.csh') is None:
            log_and_raise('did not find timavg.csh')
            ...
            for month_index in month_indices:
                ...
                with Popen(timavgcsh_command,
                    ...
                    if subp.returncode != 0:
                        ...
                        log_and_raise(f'error: timavg.csh had a problem, subp.returncode = {subp.returncode}')
        ...
        with Popen(timavgcsh_command,
            ...
            if subp.returncode != 0:
                fre_logger.error('stderror  = %s', stderror)
                log_and_raise(f'error: timavgcsh command not properly executed, subp.returncode = {subp.returncode}')
