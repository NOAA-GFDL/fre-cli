...
class frenctoolsTimeAverager(timeAverager):
    ...
    def generate_timavg(self, infile=None, outfile=None):
        ...
        if self.var is not None:
            fre_logger.warning(
                "Variable specification (var= %s)" + " not currently supported for frenctols time averaging. ignoring!",
                self.var,
            )
        ...
        if self.avg_type == "month":
            ...
            nc_month_file_paths = {
                month_index: os.path.join(monthly_nc_dir, f"all_years.{month_index}.nc")
                for month_index in month_indices
            }
            month_output_file_paths = {
                month_index: os.path.join(output_dir, f"{outfile}_.{month_index}.nc") for month_index in month_indices
            }
            ...
            for month_index in month_indices:
                ...
                exitstatus = 1
                with Popen(timavgcsh_command, stdout=PIPE, stderr=PIPE, shell=False) as subp:
