...
def run_avgtype_pkg_calculations(infile=None, outfile=None, pkg=None, avg_type=None, unwgt=None):
    ...
    assert all([infile is not None, outfile is not None, pkg is not None, avg_type is not None, unwgt is not None])
    ...
    gtas.generate_time_average(infile=infile, outfile=outfile, pkg=pkg, unwgt=unwgt, avg_type=avg_type)
...
def test_compare_fre_cli_to_fre_nctools():
    ...
    try:
        ...
        print(
            "fre-nctools input file not found.         probably because you are not at GFDL! run the shell script      "
            "   example if you would like to see this pass. otherwise,         i will error right after this message."
        )
        try:
    ...
    assert all(
        [
            len(fre_pytools_timavg) == len(fre_nctools_timavg),
...
def test_compare_fre_cli_to_cdo():
    ...
    assert all(
        [
            len(fre_pytools_timavg) == len(cdo_timavg),
    ...
    assert not ((non_zero_count > 0.0) or (non_zero_count < 0.0))
...
def test_compare_cdo_to_fre_nctools():
    ...
    assert all(
        [
            len(cdo_timavg) == len(fre_nctools_timavg),
...
def test_fre_cli_app_gen_time_avg_cleanup():
    ...
    nc_files = [os.path.join(time_avg_file_dir, el) for el in os.listdir(time_avg_file_dir) if el.endswith(".nc")]
