...
@click.option("--rose_config", type=str, help="Path to Rose app configuration (to be removed soon)", required=True)
def regrid(input_dir, output_dir, begin, tmp_dir, remap_dir, source, grid_spec, rose_config):
    """regrid target netcdf file"""
    regrid_xy(input_dir, output_dir, begin, tmp_dir, remap_dir, source, grid_spec, rose_config)


@app_cli.command()
@click.option(
    "-i", "--infile", type=str, help="Input NetCDF file containing pressure-level output to be masked", required=True
)
@click.option("-o", "--outfile", type=str, help="Output file", required=True)
@click.option(
    "-p",
    "--psfile",  # surface pressure... ps? TODO
...
@app_cli.command()
@click.option("-i", "--inf", type=str, required=True, help="Input file name")
@click.option("-o", "--outf", type=str, required=True, help="Output file name")
@click.option(
    "-p",
    "--pkg",
...
@click.option("-v", "--var", type=str, default=None, help="Specify variable to average")
@click.option("-u", "--unwgt", is_flag=True, default=False, help="Request unweighted statistics")
@click.option(
    "-a",
    "--avg_type",
    ...
    default="all",
    help=(
        "Type of time average to generate. \n                      currently, fre-nctools and fre-python-tools pkg"
