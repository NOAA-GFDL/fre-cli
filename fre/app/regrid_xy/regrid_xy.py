...
def truncate_date(date, freq):
    ...
    output = subprocess.Popen(["cylc", "cycle-point", "--template", format_, date], stdout=subprocess.PIPE)
...
def check_interp_method(nc_variable, interp_method):
    ...
    if "interp_method" not in attr_list:
        ...
        fre_logger.info(
            f"WARNING: variable '{nc_variable.name}' has attribute interp_method '{nc_variable.interp_method}'"
        )
...
def check_per_component_settings(component_list, rose_app_cfg):
    ...
    curr_out_grid_type_list = [safe_rose_config_get(rose_app_cfg, component_list[0], "outputGridType")]
...
def make_regrid_var_list(target_file, interp_method=None):
    ...
    for var_name in all_fin_vars:
        if var_name in ["average_T1", "average_T2", "average_DT", "time_bnds"]:
            continue
...
def regrid_xy(input_dir, output_dir, begin, tmp_dir, remap_dir, source, grid_spec, rose_config):
    """
    calls fre-nctools' fregrid to regrid netcdf files
    """

    # mandatory arguments- code exits if any of these are not present
    if None in [input_dir, output_dir, begin, tmp_dir, remap_dir, source, grid_spec, rose_config]:
    ...
    if not Path(output_dir).exists():
        raise OSError("the following does not exist and/or could not be created:" + f"output_dir=\n{output_dir}")

    # work/ dir check
    work_dir = tmp_dir + "work/"
    Path(work_dir).mkdir(exist_ok=True)
    if not Path(work_dir).exists():
        raise OSError("the following does not exist and/or could not be created:" + f"work_dir=\n{work_dir}")
    ...
    os.chdir(work_dir)
    if ".tar" in grid_spec:
        untar_sp = subprocess.run(["tar", "-xvf", grid_spec], check=True, capture_output=True)
    ...
    for component in component_list:
        if not do_regridding[component_list.index(component)]:
            continue
        ...
        target_file += (
            f"/{truncate_date(begin,'P1D')}.{source}.tile1.nc"
            if is_tiled
        ...
        fre_logger.info(
            f"output_grid_type = {output_grid_type }\n"
        ...
        if remap_file is not None:
            try:
                ...
                raise OSError("remap_file={remap_file} could not be copied to local dir") from exc
        else:
            remap_file = f"fregrid_remap_file_{output_grid_lon}_by_{output_grid_lat}.nc"
            remap_cache_file = (
                f"{remap_dir}/{input_grid}/{input_realm}/" + f"{source_nx}-by-{source_ny}/{interp_method}/{remap_file}"
            )
            ...
            if Path(remap_cache_file).exists():
                ...
                shutil.copy(remap_cache_file, remap_cache_file.split("/").pop())
            else:
        ...
        input_file = target_file.replace(".tile1.nc", "") if ".tile1" in target_file else target_file
        input_file = input_file.split("/").pop()

        # create output file argument...
        output_file = target_file.replace(".tile1", "") if "tile1" in target_file else target_file
        ...
        fregrid_command = [
            ...
            f"{remap_file}",
            "--nlon",
            f"{str(output_grid_lon)}",
            "--nlat",
            f"{str(output_grid_lat)}",
            "--scalar_field",
            f"{regrid_vars_str}",
            "--output_file",
            f"{output_file}",
        ...
        final_output_dir = output_dir if output_grid_type is None else output_dir + "/" + output_grid_type
