...
def test_untar_inputs():
    ...
    assert all([sp.returncode == 0, Path(IN_DIR).exists()])
...
def test_make_ncgen3_nc_inputs():
    ...
    if Path(ncgen3_output).exists():
        ...
        assert all([sp.returncode == 0, Path(ncgen3_output).exists()])
...
def test_make_ncgen_tile_nc_inputs():
    ...
    if the output exists, it will not bother remaking it
    """
    ncgen_tile_i_nc_output_exists = [
        Path(WORK_YYYYMMDD_DIR + f"{YYYYMMDD}.{SOURCE}.tile{i}.nc").exists() for i in range(1, 6 + 1)
    ]
    if all(ncgen_tile_i_nc_output_exists):
        ...
        for i in range(1, 6 + 1):
            ...
            assert all([sp.returncode == 0, Path(ncgen_tile_i_nc_output).exists()])
...
def test_make_ncgen_grid_spec_nc_inputs():
    ...
    if the output exists, it will not bother remaking it
    """
    ncgen_grid_spec_i_nc_output_exists = [
        Path(WORK_YYYYMMDD_DIR + f"{YYYYMMDD}.grid_spec.tile{i}.nc").exists() for i in range(1, 6 + 1)
    ]
    if all(ncgen_grid_spec_i_nc_output_exists):
        ...
        for i in range(1, 6 + 1):
            ...
            assert all([sp.returncode == 0, Path(ncgen_grid_spec_i_nc_output).exists()])
...
def test_make_hgrid_gold_input():
    ...
    """
    grid_i_file_targ_loc_exists = [Path(WORK_YYYYMMDD_DIR + f"C96_grid.tile{i}.nc").exists() for i in range(1, 6 + 1)]
    if all(grid_i_file_targ_loc_exists):
        ...
        for i in range(1, 6 + 1):
            ...
            assert all([sp.returncode == 0, Path(grid_i_file_targ_loc).exists()])
...
def test_make_fregrid_comparison_input():
    ...
    ex = [
        ...
        "--standard_dimension",
        "--input_mosaic",
        fregrid_input_mosaic_arg,
        "--input_dir",
        fregrid_input_dir_arg,
        "--input_file",
        fregrid_input_file_arg,
        "--associated_file_dir",
        fregrid_assoc_file_dir_arg,
        "--remap_file",
        fregrid_remap_file_arg,
        "--nlon",
        fregrid_nlon_arg,
        "--nlat",
        fregrid_nlat_arg,
        ...
        fregrid_vars_arg,
        "--output_file",
        fregrid_output_file_arg,
    ]
    print(" \n".join(ex))
    sp = subprocess.run(ex, check=True)

    assert all([sp.returncode == 0, Path(fregrid_remap_file_arg).exists(), Path(fregrid_output_file_arg).exists()])
...
def test_import_regrid_xy():
    ...
    """
    assert all([rgxy is not None, rgxy.test_import() == 1])
...
def test_success_no_tar_grid_spec_regrid_xy():
    ...
    assert rgxy_returncode == 0
    assert Path(
        REMAP_DIR + f"{INPUT_GRID}/{INPUT_REALM}/96-by-96/{INTERP_METHOD}/" + f"fregrid_remap_file_{NLON}_by_{NLAT}.nc"
    ).exists()
    assert Path(TEST_OUT_DIR).exists()
    assert Path(TEST_OUT_DIR + f"{YYYYMMDD}.{SOURCE}.nc").exists()
    assert Path(WORK_DIR).exists()
...
def test_nccmp1_regrid_xy():
    ...
    nccmp_arg1 = (
        REMAP_DIR + INPUT_GRID + "/" + INPUT_REALM + "/" + SOURCES_XY + "/" + INTERP_METHOD + "/" + fregrid_remap_file
    )
