...
def gridspec():
    ...
    xcomponents = {
        "aXo": ["atmXocn_exchange_grid_file", aXo_files],
...
def mosaic():
    ...
    data["contact_index"] = xr.DataArray(
        contact_indexlist, attrs={"standard_name": "starting_ending_point_index_of_contact"}, dims=["ncontact"]
    )
...
def grid():
    ...
    for i in range(1, 7):
        ...
        # x variable
        data["x"] = xr.DataArray(
            x, dims=["nyp", "nxp"], attrs={"standard_name": "geographic_longitude", "units": "degree_east"}
        )

        # y variable
        data["y"] = xr.DataArray(
            y, dims=["nyp", "nxp"], attrs={"standard_name": "geographic_longitude", "units": "degree_north"}
        )
