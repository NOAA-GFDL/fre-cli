...
def test_checkout_script_exists():
    ...
    create_checkout_script.checkout_create(
        YAMLFILE, PLATFORM, TARGET, no_parallel_checkout=False, jobs=False, execute=False, verbose=False
    )
...
def test_checkout_verbose():
    ...
    """
    create_checkout_script.checkout_create(
        YAMLFILE, PLATFORM, TARGET, no_parallel_checkout=False, jobs=False, execute=False, verbose=True
    )
...
def test_checkout_execute():
    ...
    shutil.rmtree(f"{OUT}/fremake_canopy/test", ignore_errors=True)
    create_checkout_script.checkout_create(
        YAMLFILE, PLATFORM, TARGET, no_parallel_checkout=False, jobs=2, execute=True, verbose=False
    )
...
def test_checkout_no_parallel_checkout():
    ...
    """
    create_checkout_script.checkout_create(
        YAMLFILE, PLATFORM, TARGET, no_parallel_checkout=True, jobs=False, execute=False, verbose=False
    )
