...
def test_run_fremake_serial_compile():
    ...
    run_fremake_script.fremake_run(
        ...
        no_format_transfer=True,
        execute=True,
        verbose=VERBOSE,
    )
    assert Path(
        f"{SERIAL_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{TARGET[0]}/exec/{EXPERIMENT}.x"
    ).exists()
...
def test_run_fremake_multijob_compile():
    ...
    assert Path(
        f"{MULTIJOB_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{TARGET[0]}/exec/{EXPERIMENT}.x"
    ).exists()
...
def test_run_fremake_container_build_specified_out():
    ...
    assert Path(f"{CONTAINER_BUILD_TEST_PATH}/fremake_canopy/test/null_model_full-debug.sif").exists()
