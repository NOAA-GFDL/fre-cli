...
def test_bad_platform_option():
    ...
    create_docker_script.dockerfile_create(YAMLPATH, BADOPT, TARGET, execute=False, skip_format_transfer=False)


@pytest.mark.xfail()
def test_bad_target_option():
    """test create-dockerfile with a invalid target option"""
    create_docker_script.dockerfile_create(YAMLPATH, PLATFORM, BADOPT, execute=False, skip_format_transfer=False)


@pytest.mark.xfail()
def test_bad_yamlpath_option():
    """test create-dockerfile with a invalid target option"""
    create_docker_script.dockerfile_create(BADOPT[0], PLATFORM, TARGET, execute=False, skip_format_transfer=False)
...
def test_no_op_platform():
    ...
    create_docker_script.dockerfile_create(YAMLPATH, ["ci.gnu"], TARGET, execute=False, skip_format_transfer=False)
...
def test_create_dockerfile():
    ...
    create_docker_script.dockerfile_create(YAMLPATH, PLATFORM, TARGET, execute=False, skip_format_transfer=False)
