...
def test_run_fremake_compile_script_creation_serial():
    ...
    assert Path(
        f"{SERIAL_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{TARGET[0]}/exec/compile.sh"
    ).exists()
...
def test_run_fremake_compile_script_creation_multijob():
    """check for compile script creation from previous test"""
    assert Path(
        f"{MULTIJOB_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{TARGET[0]}/exec/compile.sh"
    ).exists()
...
def test_run_fremake_makefile_creation_multijob():
    """check for makefile creation from previous test"""
    assert Path(
        f"{MULTIJOB_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{TARGET[0]}/exec/Makefile"
    ).exists()
...
def test_run_fremake_compile_script_creation_multitarget():
    ...
    for t in targets:
        assert Path(
            f"{MULTITARGET_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{t}/exec/compile.sh"
        ).exists()
...
def test_run_fremake_makefile_creation_multitarget():
    ...
    for t in targets:
        assert Path(
            f"{MULTITARGET_TEST_PATH}/fremake_canopy/test/{EXPERIMENT}/{PLATFORM[0]}-{t}/exec/Makefile"
        ).exists()
