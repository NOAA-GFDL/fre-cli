...
class container:
    ...
    def __init__(self, base, exp, libs, RUNenv, target, mkTemplate, stage2base):
        ...
        if self.stage2base == "":
            ...
            self.secondstage = [
                f"FROM {self.stage2base} as final\n",
    ...
    def writeDockerfileMkmf(self, c):
        ...
        if (
            c["requires"] == [] and c["doF90Cpp"]
        ):  # If this lib doesnt have any code dependencies and it requires the preprocessor (no -o and yes --use-cpp)
            self.d.write(
    ...
    def createBuildScript(self, platform, skip_format_transfer):
        ...
        if not skip_format_transfer:
            ...
            self.userScript.append(
                f"{containerBuild} save -o {containerName}.tar localhost/{self.e}:{self.target.gettargetName()}\n"
            )
            self.userScript.append(
