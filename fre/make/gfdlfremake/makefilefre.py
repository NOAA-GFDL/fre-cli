...
# CONTAINER; write a script that will execute in the container, to fill in
# link line with additional libraries in Makefile
    ...
    def writeMakefile(self):
        ...
        with open(self.filePath + "/Makefile", "a") as fh:
            for (c, r, o) in sd:
                ...
                if o == "":
                    fh.write(
                        "\t$(MAKE) SRCROOT=$(SRCROOT) BUILDROOT=$(BUILDROOT) MK_TEMPLATE=$(MK_TEMPLATE) --directory=" +
                        c +
                        " $(@F)\n")
                else:
                    fh.write(
                        "\t$(MAKE) SRCROOT=$(SRCROOT) BUILDROOT=$(BUILDROOT) MK_TEMPLATE=$(MK_TEMPLATE) " +
                        o +
                        " --directory=" +
                        c +
                        " $(@F)\n")
