...
# Constants
ACCEPTED_VERT_DIMS = ["z_l", "landuse", "plev39", "plev30", "plev19", "plev8", "height2m", "level", "lev", "levhalf"]
...
def rewrite_netcdf_file_var(
    ...
    try:
        ...
        fre_logger.warning(
            "could not get expected coordinate dimensions for %s.    in mip_var_cfgs file %s. \n exc = %s",
            target_var,
            json_table_config,
            exc,
        )
    ...
    if process_tripolar_data:
        ...
        if any([yh_dim != (yq_dim - 1), xh_dim != (xq_dim - 1)]):
    ...
    try:
        ...
        cmor_time = cmor.axis("time", coord_vals=time_coords, cell_bounds=time_bnds, units=time_coord_units)
    except ValueError as exc:  # uncovered
        fre_logger.info(
            "cmor_time = cmor.axis('time', \n    coord_vals = %s, units = %s)", time_coords, time_coord_units
        )
    ...
    if lev is not None:
        ...
        if vert_dim.lower() in NON_HYBRID_SIGMA_COORDS:
            ...
            if vert_dim.lower() != "landuse":
                ...
                cmor_z = cmor.axis(cmor_vert_dim_name, coord_vals=lev[:], units=lev_units)
            else:
                landuse_str_list = ["primary_and_secondary_land", "pastures", "crops", "urban"]
                cmor_vert_dim_name = "landUse"
                cmor_z = cmor.axis(
                    cmor_vert_dim_name,
                    coord_vals=np.array(landuse_str_list, dtype=f"S{len(landuse_str_list[0])}"),
                    units=lev_units,
                )
            ...
            cmor_z = cmor.axis("depth_coord", coord_vals=lev[:], units=lev_units, cell_bounds=lev_bnds)
            ...
            if vert_dim == "levhalf":
                cmor_z = cmor.axis("alternate_hybrid_sigma_half", coord_vals=lev[:], units=lev_units)
                ierr_ap = cmor.zfactor(
                    zaxis_id=cmor_z,
                ...
                cmor_z = cmor.axis(
                    "alternate_hybrid_sigma", coord_vals=lev[:], units=lev_units, cell_bounds=ds[vert_dim + "_bnds"]
                )
                ierr_ap = cmor.zfactor(
                    zaxis_id=cmor_z,
                    zfactor_name="ap",
            ...
            ips = cmor.zfactor(zaxis_id=cmor_z, zfactor_name="ps", axis_ids=axis_ids, units="Pa")
            save_ps = True
    ...
    if save_ps:
        if any([ips is None, ps is None]):
            fre_logger.warning(
                "ps or ips is None!, but save_ps is True!\n"  # uncovered
                "ps = %s, ips = %s\n"
                "skipping ps writing!",
                ps,
                ips,
...
def cmorize_target_var_files(
    indir=None,
    target_var=None,
    local_var=None,
    iso_datetime_range_arr=None,
    name_of_set=None,
    json_exp_config=None,
    outdir=None,
    mip_var_cfgs=None,
    ...
    """
    fre_logger.info(
        "local_var = %s to be used for file-targeting.\n"
    ...
    for i, iso_datetime in enumerate(iso_datetime_range_arr):
        ...
        try:
            local_file_name = rewrite_netcdf_file_var(
                mip_var_cfgs, local_var, nc_file_work, target_var, json_exp_config, json_table_config, nc_fls[i]
            )
        except Exception as exc:  # uncovered
            raise Exception(
                f"problem with rewrite_netcdf_file_var. exc={exc}\nexiting and executing finally block."
            ) from exc
...
def cmorize_all_variables_in_dir(
    vars_to_run,
    indir,
...
def cmor_run_subtool(
    ...
    if None in [indir, json_var_list, json_table_config, json_exp_config, outdir]:
        raise ValueError(
            "all input arguments except opt_var_name are required!\n"  # uncovered
    ...
    if any([grid_label is not None, grid is not None, nom_res is not None]):
        update_grid_and_label(json_exp_config, grid_label, grid, nom_res, output_file_path=None)
    ...
    if Path(json_exp_config).exists():
        ...
        raise FileNotFoundError(
            "ERROR: json_exp_config file cannot be opened.\n"  # uncovered
            "json_exp_config = %s",
            json_exp_config,
        )
    ...
    if len(vars_to_run) < 1:
        raise ValueError(
            "runnable variable list is of length 0"  # uncovered
        ...
        raise ValueError(
            "opt_var_name is not None! (== %s)"  # uncovered
    ...
    return cmorize_all_variables_in_dir(
        vars_to_run,
        indir,
        iso_datetime_range_arr,
        name_of_set,
