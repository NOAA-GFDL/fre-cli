...
OPT_VAR_NAME_HELP = (
    "optional, specify a variable name to specifically process only filenames "
    + "matching that variable name. I.e., this string help target local_vars, not "
    + "target_vars."
)
VARLIST_HELP = (
    "path pointing to a json file containing directory of key/value pairs. "
...
START_YEAR_HELP = (
    "string representing the minimum calendar year CMOR should start processing for. "
    + "currently, only YYYY format is supported."
)
STOP_YEAR_HELP = (
    "string representing the maximum calendar year CMOR should stop processing for. "
    + "currently, only YYYY format is supported."
)
...
@click.option("-o", "--output", type=str, default=None, help="Output file if desired", required=False)
@click.option("--run_one", is_flag=True, default=False, help=RUN_ONE_HELP, required=False)
@click.option("--dry_run", is_flag=True, default=False, help=DRY_RUN_HELP, required=False)
@click.option("--start", type=str, default=None, help=START_YEAR_HELP, required=False)
@click.option("--stop", type=str, default=None, help=STOP_YEAR_HELP, required=False)
...
@cmor_cli.command()
@click.option("-l", "--varlist", type=str, help=VARLIST_HELP, required=False)
@click.option(
    "-r",
    "--table_config_dir",
    type=str,
...
@click.option("-v", "--opt_var_name", type=str, help=OPT_VAR_NAME_HELP, required=False)
def find(varlist, table_config_dir, opt_var_name):  # uncovered
    ...
    cmor_find_subtool(json_var_list=varlist, json_table_config_dir=table_config_dir, opt_var_name=opt_var_name)
...
@click.option("-l", "--varlist", type=str, help=VARLIST_HELP, required=True)
@click.option(
    "-r",
    "--table_config",
    type=str,
...
@click.option("--run_one", is_flag=True, default=False, help=RUN_ONE_HELP, required=False)
@click.option("-v", "--opt_var_name", type=str, default=None, help=OPT_VAR_NAME_HELP, required=False)
@click.option(
    "-g",
    "--grid_label",
    type=str,
...
@click.option("--stop", type=str, default=None, help=STOP_YEAR_HELP, required=False)
def run(
    indir, varlist, table_config, exp_config, outdir, run_one, opt_var_name, grid_label, grid_desc, nom_res, start, stop
):
