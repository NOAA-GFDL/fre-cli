...
def iso_to_bronx_chunk(cmor_chunk_in):
    ...
    if cmor_chunk_in[0] == "P" and cmor_chunk_in[-1] == "Y":
        ...
        raise ValueError(
            "problem with converting to bronx chunk from the cmor chunk. check cmor_yamler.py"
        )  # uncovered
    ...
    with open(json_table_config, "r", encoding="utf-8") as table_config_file:
        ...
        for var_entry in table_config_data["variable_entry"]:
            try:
                ...
                raise KeyError(
                    "could not get freq from table!!! variable entries in cmip cmor tables"
...
def cmor_yaml_subtool(
    ...
    cmor_yaml_dict = consolidate_yamls(
        yamlfile=yamlfile, experiment=exp_name, platform=platform, target=target, use="cmor", output=output
    )["cmor"]
    fre_logger.debug(
        "consolidate_yamls produced the following dictionary of cmor-settings from yamls: \n%s",
    ...
    pp_dir = os.path.expandvars(cmor_yaml_dict["directories"]["pp_dir"])
    fre_logger.info("pp_dir = %s", pp_dir)
    check_path_existence(pp_dir)

    # directory holding mip table config inputs
    cmip_cmor_table_dir = os.path.expandvars(cmor_yaml_dict["directories"]["table_dir"])
    ...
    cmorized_outdir = os.path.expandvars(cmor_yaml_dict["directories"]["outdir"])
    ...
    json_exp_config = os.path.expandvars(cmor_yaml_dict["exp_json"])
    ...
    for table_config in cmor_yaml_dict["table_targets"]:
        ...
        json_var_list = os.path.expandvars(table_config["variable_list"])
