...
def print_var_content( table_config_file = None, opt_var_name = None):
    ''' one variable printing routine '''
    try:
    ...
    var_content = None
    try:
        var_content = proj_table_vars["variable_entry"].get(opt_var_name)
    except:
    ...
    if var_content is None:
        #print(f'(cmor_list_subtool) variable {opt_var_name} not found in {Path(json_table_config).name}, moving on!')
        return
    
    table_name = None
    try:
        print(f'(print_var_content) trying to get table_name from proj_table_vars...')
        table_name = proj_table_vars["Header"].get(table_name)
    except:
        print(f'print_var_content) WARNING couldnt get header and table_name field')
        pass

    print(f'(print_var_content) found {opt_var_name} data in table!\n')
    if table_name is not None:
        print(f'           table_name = {table_name}')
        
    print(f'                    {opt_var_name} stuff is = \n')
