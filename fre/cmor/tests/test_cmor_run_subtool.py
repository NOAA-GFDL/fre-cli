...
TABLE_CONFIG = f"{CMIP6_TABLE_REPO_PATH}/Tables/CMIP6_Omon.json"


def test_setup_cmor_cmip_table_repo():
    """setup routine, make sure the recursively cloned tables exist"""
    assert all([Path(CMIP6_TABLE_REPO_PATH).exists(), Path(TABLE_CONFIG).exists()])
...
CMOR_CREATES_DIR = f"CMIP6/CMIP6/ISMIP6/PCMDI/PCMDI-test-1-0/piControl-withism/r3i1p1f1/Omon/sos/{GRID_LABEL}"
FULL_OUTPUTDIR = f"{OUTDIR}/{CMOR_CREATES_DIR}/v{YYYYMMDD}"
FULL_OUTPUTFILE = f"{FULL_OUTPUTDIR}/sos_Omon_PCMDI-test-1-0_piControl-withism_r3i1p1f1_{GRID_LABEL}_199307-199308.nc"
...
def test_fre_cmor_run_subtool_case1(capfd):
    ...
    assert all([Path(FULL_OUTPUTFILE).exists(), Path(FULL_INPUTFILE).exists()])
    _out, _err = capfd.readouterr()
...
def test_fre_cmor_run_subtool_case1_output_compare_data(capfd):
    ...
    nccmp_cmd = ["nccmp", "-f", "-d", f"{FULL_INPUTFILE}", f"{FULL_OUTPUTFILE}"]
    ...
    assert all([result.returncode == 1, len(err_list) == 2, "" in err_list, expected_err in err_list])
    _out, _err = capfd.readouterr()
...
def test_fre_cmor_run_subtool_case1_output_compare_metadata(capfd):
    ...
    nccmp_cmd = ["nccmp", "-f", "-m", "-g", f"{FULL_INPUTFILE}", f"{FULL_OUTPUTFILE}"]
    print(f"via subprocess, running {' '.join(nccmp_cmd)}")
...
FILENAME_DIFF = "reduced_ocean_monthly_1x1deg.199307-199308.sosV2.nc"
...
def test_setup_fre_cmor_run_subtool_case2(capfd):
    ...
    if not Path(FULL_INPUTFILE_DIFF).exists():
        shutil.copy(Path(FULL_INPUTFILE), Path(FULL_INPUTFILE_DIFF))
...
def test_fre_cmor_run_subtool_case2(capfd):
    ...
    assert all([Path(FULL_OUTPUTFILE).exists(), Path(FULL_INPUTFILE_DIFF).exists()])
    _out, _err = capfd.readouterr()
...
def test_fre_cmor_run_subtool_case2_output_compare_data(capfd):
    ...
    nccmp_cmd = ["nccmp", "-f", "-d", f"{FULL_INPUTFILE_DIFF}", f"{FULL_OUTPUTFILE}"]
    print(f"via subprocess, running {' '.join(nccmp_cmd)}")
    result = subprocess.run(" ".join(nccmp_cmd), shell=True, check=False, capture_output=True)
...
def test_fre_cmor_run_subtool_case2_output_compare_metadata(capfd):
    ...
    nccmp_cmd = ["nccmp", "-f", "-m", "-g", f"{FULL_INPUTFILE_DIFF}", f"{FULL_OUTPUTFILE}"]
    print(f"via subprocess, running {' '.join(nccmp_cmd)}")
    result = subprocess.run(" ".join(nccmp_cmd), shell=True, check=False)
