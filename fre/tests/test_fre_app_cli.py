...
def test_cli_fre_app_regrid_test_case_1(capfd):
    ...
    if not Path(t_rgxy.TEST_DIR + "/in-dir").exists():
        ...
        assert all([sp.returncode == 0, Path(t_rgxy.IN_DIR).exists()])
    ...
    args_list = [
        ...
        "regrid",
        "--input_dir",
        f"{t_rgxy.WORK_YYYYMMDD_DIR}",
        "--output_dir",
        f"{t_rgxy.TEST_OUT_DIR}",
        "--begin",
        f"{t_rgxy.YYYYMMDD}T000000",
        "--tmp_dir",
        f"{t_rgxy.TEST_DIR}",
        "--remap_dir",
        f"{t_rgxy.REMAP_DIR}",
        ...
        f"{t_rgxy.SOURCE}",
        "--grid_spec",
        f"{t_rgxy.GOLD_GRID_SPEC_NO_TAR}",
        "--def_xy_interp",
        f'"{t_rgxy.NLON},{t_rgxy.NLAT}"',
    ]
