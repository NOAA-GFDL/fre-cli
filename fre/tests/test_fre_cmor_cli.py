...
def test_cli_fre_cmor_yaml_case1():
    ...
    if os.access("/archive", os.W_OK):
        Path(
            os.path.expandvars("/archive/$USER/am5/am5f7b12r1/c96L65_am5f7b12r1_amip/ncrc5.intel-prod-openmp/pp")
        ...
        assert all(
            [
                Path(TEST_AM5_YAML_PATH).exists(),  # input, unparsed, model-yaml file
...
def test_cli_fre_cmor_run_case1():
    ...
    full_outputdir = f"{outdir}/{cmor_creates_dir}/v{YYYYMMDD}"  # yay no more 'fre' where it shouldnt be
    full_outputfile = (
    ...
    assert all([result.exit_code == 0, Path(full_outputfile).exists(), Path(full_inputfile).exists()])
