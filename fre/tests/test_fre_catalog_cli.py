...
def test_cli_fre_catalog_builder():
    ...
    stdout_str = "Missing: input_path or output_path. " + "Pass it in the config yaml or as command-line option"
    assert all([result.exit_code == 1, stdout_str in result.stdout.split("\n")])
...
def test_cli_fre_catalog_merge():
    ...
    assert all([result.exit_code == 2, expected_stdout in result.stdout.split("\n")])
