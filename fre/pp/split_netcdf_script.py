...
def split_netcdf(inputDir, outputDir, component, history_source, use_subdirs, yamlfile, split_all_vars=False):
    """
    ...
    if use_subdirs:
        ...
        if files_split == 0:
            fre_logger.error(
                f"error: no files found in dirs {sd_string} under {workdir} that match pattern {file_regex}; no"
...
def set_coord_encoding(dset, vcoords):
    ...
    for vc in vcoords:
        ...
        encode_dict[vc] = {"_FillValue": None, "dtype": dset[vc].encoding["dtype"]}
...
def set_var_encoding(dset, varnames):
    ...
    for v in varnames:
        ...
        if not "_FillValue" in v_encoding.keys():
            encode_dict[v] = {"_FillValue": None, "dtype": dset[v].encoding["dtype"]}
...
def parse_yaml_for_varlist(yamlfile, yamlcomp, hist_source="none"):
    ...
    if hist_source != "none":
        ...
        if not any([el == hist_source for el in hist_srces]):
            fre_logger.error(
                f"error in parse_yaml_for_varlist: history_file {hist_source} is not found under component"
                f" {yamlcomp} in file {yamlfile}"
            )
            raise ValueError(
                f"error in parse_yaml_for_varlist: history_file {hist_source} is not found under component"
                f" {yamlcomp} in file {yamlfile}"
...
def parse_yaml_for_varlist_ppcompstyle(yamlfile, is_static, hist_source):
    ...
    if varlist is None:
        fre_logger.error(
            f"error in parse_yaml_for_varlist_ppcompstle: history_file {hist_source} was not found in file {yamlfile}"
        )
        raise ValueError
    return varlist
...
def get_variables(comp_info, product, req_source):
    ...
    if product == "static":
        if comp_info.get("static") is None:
            raise ValueError(
                f"Product is set to static but no static sources/variables defined for {comp_info.get('type')}"
            )
