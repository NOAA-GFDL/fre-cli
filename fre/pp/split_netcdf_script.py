...
def split_netcdf(inputDir, outputDir, component, history_source, use_subdirs,
    ...
    if use_subdirs:
        ...
        if files_split == 0:
            fre_logger.error(
                f"error: no files found in dirs {sd_string} under {workdir} that match pattern {file_regex}; no splitting took place")
            raise OSError
    else:
...
def parse_yaml_for_varlist(yamlfile, yamlcomp, hist_source="none"):
    ...
    if hist_source != "none":
        ...
        if not any([el == hist_source for el in hist_srces]):
            fre_logger.error(
                f"error in parse_yaml_for_varlist: history_file {hist_source} is not found under component {yamlcomp} in file {yamlfile}")
            raise ValueError(
                f"error in parse_yaml_for_varlist: history_file {hist_source} is not found under component {yamlcomp} in file {yamlfile}")
