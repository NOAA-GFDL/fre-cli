...
def getenot(date_start, date_end, chunk_type, cal):
    ...
    if chunk_type == 'daily':
        start = cftime.datetime(int(date_start[1]), int(date_start[2].lstrip('0')),
                                int(date_start[3].lstrip('0')), calendar=cal)
        end = cftime.datetime(int(date_end[1]), int(date_end[2].lstrip('0')),
                              int(date_end[3].lstrip('0')), calendar=cal)
        diff = end - start
        enot = diff.days + 1

    if chunk_type == '4xdaily':
        start = cftime.datetime(int(date_start[1]), int(date_start[2].lstrip('0')),
                                int(date_start[3].lstrip('0')), hour=int(date_start[4]))
        end = cftime.datetime(int(date_end[1]), int(date_end[2].lstrip('0')),
                              int(date_end[3].lstrip('0')), hour=int(date_end[4]))
        diff = end - start
        enot = (diff.days + 1) * 4
    ...
    if chunk_type == '30minute':
        start = cftime.datetime(int(date_start[1]), int(date_start[2].lstrip('0')),
                                int(date_start[3].lstrip('0')), hour=int(date_start[4]))
        end = cftime.datetime(int(date_end[1]), int(date_end[2].lstrip('0')), int(
            date_end[3].lstrip('0')), hour=int(date_end[4]), minute=int(date_end[5]))
        diff = end - start
        enot = (diff.days + 1) * 48
...
def validate(filepath):
    ...
    # If month, day, hour, and minute strings are present it will identify
    # them by looking for groups of two digits after the year string
    match = re.compile(
    ...
    if enot is None:
        ...
        # Path elements contains the directories from the filepath.. we use this
        # to determine frequency/chunk_size in sub-daily files
        ...
        if all(freq not in path_elements for freq in expected_frequencies):
            raise ValueError(
                f" Cannot determine frequency from {filepath}. Sub-daily files must at minimum be placed in a directory corresponding to data frequency: '6hr, 'PT6H', '3hr, 'PT3H', '1hr, 'PT1H', '30min, 'PT30M, 'PT0.5H'")
