...
def chunk_from_legacy(legacy_chunk):
    ...
    if not match:
        fre_logger.error("Could not convert Bronx chunk to ISO8601 duration: %s", legacy_chunk)
        raise ValueError
...
def frelist_xpath(
    ...
    if process.returncode > 0:
        raise subprocess.CalledProcessError(
            returncode=process.returncode, cmd=cmd, output=process.stdout.strip(), stderr=process.stderr.strip()
        )
    return result
...
def main(
    ...
    rose_suite.set(
        keys=["template variables", "EXPERIMENT"], value=f"'{experiment}'"
    )  # value=f"'{}'".format(experiment))
    rose_suite.set(keys=["template variables", "PLATFORM"], value=f"'{platform}'")  # value="'{}'".format(platform))
    rose_suite.set(keys=["template variables", "TARGET"], value=f"'{target}'")  # value="'{}'".format(target))
    ...
    if historydir is None:
        ...
        fetch_history_process = subprocess.run(
            fetch_history_cmd, shell=True, check=True, capture_output=True, universal_newlines=True
        )
    ...
    if ppdir is None:
        ...
        fetch_pp_process = subprocess.run(
            fetch_pp_cmd, shell=True, check=True, capture_output=True, universal_newlines=True
        )
    ...
    fetch_analysis_dir_process = subprocess.run(
        fetch_analysis_dir_cmd, shell=True, check=True, capture_output=True, universal_newlines=True
    )
    ...
    rose_suite.set(
        keys=["template variables", "HISTORY_DIR"], value=f"'{historydir}'"
    )  # value="'{}'".format(historydir))
    # set some dirs to something else to allow bronx dual-pps easily
    if dual:
        rose_suite.set(keys=["template variables", "PP_DIR"], value=f"'{ppdir}_canopy'")
        rose_suite.set(keys=["template variables", "ANALYSIS_DIR"], value=f"'{analysis_dir}_canopy'")
    else:
        rose_suite.set(keys=["template variables", "PP_DIR"], value=f"'{ppdir}'")
        rose_suite.set(keys=["template variables", "ANALYSIS_DIR"], value=f"'{analysis_dir}'")
    rose_suite.set(keys=["template variables", "PP_GRID_SPEC"], value=f"'{grid_spec}'")
    ...
    refine_diag_cmd = (
        f"frelist -x {xml} -p {platform} -t {target} {experiment} --evaluate postProcess/refineDiag/@script"
    )
    refine_diag_process = subprocess.run(
        refine_diag_cmd, shell=True, check=True, capture_output=True, universal_newlines=True
    )
    ...
    if list_refinediags:
        ...
        if dual:
            rose_suite.set(keys=["template variables", "#HISTORY_DIR_REFINED"], value=f"'{historydir_refined}_canopy'")
        else:
            rose_suite.set(keys=["template variables", "#HISTORY_DIR_REFINED"], value=f"'{historydir_refined}'")

        rose_suite.set(keys=["template variables", "#REFINEDIAG_SCRIPTS"], value=f"'{' '.join(list_refinediags)}'")
    ...
    if str_preanalysis is not None:
        ...
        rose_suite.set(keys=["template variables", "PREANALYSIS_SCRIPT"], value=f"'{str_preanalysis}'")
    ...
    comps = frelist_xpath(
        ...
        # verbose, quiet, dual,
        "postProcess/component/@type",
    ).split()
    rose_suite.set(keys=["template variables", "PP_COMPONENTS"], value=f"'{' '.join(sorted(comps))}'")

    segment_time = frelist_xpath(
        xml,
        platform,
        target,
        experiment,
    ...
    current_date_str = frelist_xpath(
        xml,
        platform,
        target,
        experiment,
    ...
    for comp in comps:
        ...
        if comp_source.endswith("_refined"):
            fre_logger.info("NOTE: Skipping history file %s, refineDiag is turned off by default.", comp_source)
        else:
            sources.add(comp_source)
        timeseries_count = 0

        # Get the number of TS nodes
        results = frelist_xpath(
            xml,
            platform,
            target,
            experiment,
            ...
            # do_refinediag, pp_start, pp_stop, validate,
            # verbose, quiet, dual,
            f"{pp_comp_xpath_header}/timeSeries/@freq",
        ).split()
        ...
        for i in range(1, timeseries_count + 1):
            ...
            if source.endswith("_refined"):
                fre_logger.info("NOTE: Skipping history file %s, refineDiag is turned off by default.", source)
            else:
                sources.add(source)
...
def _convert(
    ...
    if validate:
        if CYLC_PATH in os.getenv("PATH"):
            ...
            raise EnvironmentError(
                "Cannot run the validator tool because "
    ...
    if any([pp_stop is None, pp_start is None]):
        ...
        fre_logger.warning(
            "After the converter has run, please edit the default '0000' values within your rose-suite.conf."
        )
    ...
    if all([pp_start is not None, pp_stop is not None]):
        ...
        if int(pp_start) >= int(pp_stop):
            fre_logger.warning(
                "Your PP_START date is equal to or later than "
                "your PP_STOP date. Please revise these values in "
                "your configuration after the converter has run."
            )
        if any([len(pp_start) > 4, len(pp_stop) > 4, int(pp_start) <= 0, int(pp_stop) <= 0]):
            fre_logger.warning(
                "At least one of your PP_start or PP_stop years "
    ...
    if cylc_loaded:
        ...
        try:
            ...
            fre_logger.error(
                "Errant values in rose-suite.conf or other Cylc errors. "
