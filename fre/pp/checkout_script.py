...
This module manages the automated cloning and validation of post-processing Cylc
...
def checkout_template(experiment=None, platform=None, target=None, branch=None):
    ...
    if None in [experiment, platform, target]:
        ...
        raise ValueError(
            'one of these are None: experiment / platform / target = \n'
    ...
    if not checkout_exists:
        ...
        if current_tag == git_clone_branch_arg or current_branch == git_clone_branch_arg:
            fre_logger.info(
                f"checkout exists ('{directory}/{workflow_name}'), and matches '{git_clone_branch_arg}'"
            )
        else:
            fre_logger.info(
