...
# fre pp status
@pp_cli.command()
@click.option("-e", "--experiment", type=str, help="Experiment name", required=True)
@click.option("-p", "--platform", type=str, help="Platform name", required=True)
@click.option("-t", "--target", type=str, help="Target name", required=True)
...
@click.option("-p", "--platform", type=str, help="Platform name", required=True)
@click.option("-t", "--target", type=str, help="Target name", required=True)
@click.option("--pause", is_flag=True, default=False, help="Pause the workflow immediately on start up", required=False)
@click.option(
    "--no_wait",
    is_flag=True,
    default=False,
...
@pp_cli.command()
@click.option("-y", "--yamlfile", type=str, help="YAML file to be used for parsing", required=True)
@click.option("-e", "--experiment", type=str, help="Experiment name", required=True)
@click.option("-p", "--platform", type=str, help="Platform name", required=True)
@click.option("-t", "--target", type=str, help="Target name", required=True)
...
@pp_cli.command()
@click.option("-x", "--xml", required=True, help="Required. The Bronx XML")
@click.option("-p", "--platform", required=True, help="Required. The Bronx XML Platform")
@click.option("-t", "--target", required=True, help="Required. The Bronx XML Target")
@click.option("-e", "--experiment", required=True, help="Required. The Bronx XML Experiment")
@click.option("--do_analysis", is_flag=True, default=False, help="Optional. Runs the analysis scripts.")
@click.option(
    "--historydir", help="Optional. History directory to reference. If not specified, the XML's default will be used."
)
@click.option(
    "--refinedir",
    help="Optional. History refineDiag directory to reference. If not specified, the XML's default will be used.",
)
@click.option(
    "--ppdir", help="Optional. Postprocessing directory to reference. If not specified, the XML's default will be used."
)
@click.option("--do_refinediag", is_flag=True, default=False, help="Optional. Process refineDiag scripts")
@click.option(
    "--pp_start",
    type=str,
    default="0000",
    help=(
        "Optional. Starting year of postprocessing. "
...
@click.option(
    ...
    default="0000",
    help=(
        "Optional. Ending year of postprocessing. "
        "If not specified, a default value of '0000' "
        "will be set and must be changed in rose-suite.conf"
    ),
)
@click.option("--validate", is_flag=True, help="Optional. Run the Cylc validator immediately after conversion")
@click.option("-v", "--verbose", is_flag=True, help="Optional. Display detailed output")
@click.option("-q", "--quiet", is_flag=True, help="Optional. Display only serious messages and/or errors")
@click.option("--dual", is_flag=True, help="Optional. Append '_canopy' to pp, analysis, and refinediag dirs")
def configure_xml(
    xml,
    platform,
    target,
...
@click.option(
    ...
    default=False,
    help=(
        "Warn mode. Instead of raising an error, a warning will be printed in the fre log if no "
...
@click.option(
    ...
    required=True,
    help=(
        "Path to a directory in which to search for netcdf files to split. Files matching the pattern in"
        " $history-source will be split."
    ),
)
@click.option("-o", "--outputdir", required=True, help="Path to a directory to which to write split netcdf files.")
@click.option(
    "-c",
    "--component",
    required=False,
    default=None,
    help=(
        "component specified in yamlfile under postprocess:components. Needs to be the same component that contains the"
        " sources:history-file. Conflicts with --split-all-vars."
    ),
)
@click.option(
    "-s",
    ...
    default=None,
    help=(
        "history-file specification under postprocess:components:type=component:sources in the fre postprocess config"
        " yamlfile. Used to match files in inputdir."
    ),
)
@click.option(
    "-y",
    ...
    default=None,
    help=(
        "fre postprocessing .yml file from which to get the variable filtering list under"
        " postprocess:components:type=component:variables. Conflicts with --split-all-vars."
    ),
)
@click.option(
    "--use-subdirs",
    ...
    default=False,
    help=(
        "Whether to search subdirs underneath $inputdir for netcdf files. Defaults to false. This option is used in"
        " flow.cylc when regridding."
    ),
)
@click.option(
    "--split-all-vars",
    ...
    default=False,
    help=(
        "Whether to ignore other config options and split all vars in the file. Defaults to false. Conflicts with -c,"
        " -s and -y options."
    ),
)
...
Either unset --split-all-vars or parse the varlist from the yaml - do not try do do both!""")
    split_netcdf_script.split_netcdf(
        inputdir, outputdir, component, history_source, use_subdirs, yamlfile, split_all_vars
    )
...
@click.option("-p", "--platform", type=str, help="Platform name", required=True)
@click.option("-T", "--target", type=str, help="Target name", required=True)
@click.option("-c", "--config-file", type=str, help="Path to a configuration file in either XML or YAML", required=True)
@click.option(
    "-b",
    "--branch",
    required=False,
...
@click.option("-t", "--time", required=False, default=None, help="Time whose history files are ready")
...
@click.option("-p", "--platform", type=str, help="Platform name", required=True)
@click.option("-T", "--target", type=str, help="Target name", required=True)
@click.option("-t", "--time", required=True, help="Time whose history files are ready")
