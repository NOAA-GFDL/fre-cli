...
def set_rose_apps(yamlfile, rose_regrid, rose_remap):
    ...
    for i in components:
        ...
        # if xyInterp exists, component can be regridded
            ...
            rose_remap.set(
                keys=[f"{comp}", "grid"], value=f"regrid-xy/{interp_split[0]}_{interp_split[1]}.{interp_method}"
            )
        ...
        if i.get("xyInterp") is not None:
            ...
            rose_regrid.set(
                keys=[f"{comp}", "outputGridType"], value=f"{interp_split[0]}_{interp_split[1]}.{interp_method}"
            )
...
def yaml_info(yamlfile=None, experiment=None, platform=None, target=None):
    ...
    full_yamldict = cy.consolidate_yamls(yamlfile=yml, experiment=e, platform=p, target=t, use="pp", output=outfile)
