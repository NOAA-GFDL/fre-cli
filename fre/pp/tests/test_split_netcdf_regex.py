...
def test_split_netcdf_file_regex_pattern():
    """
    Test that split_netcdf function correctly creates the file regex pattern on line 74
    ...
    with tempfile.TemporaryDirectory() as temp_input, \
        ...
        with patch('fre.pp.split_netcdf_script.parse_yaml_for_varlist') as mock_parse_yaml, \
            ...
            # Setup mocks
            mock_parse_yaml.return_value = ["var1", "var2"]
...
def test_file_regex_pattern_direct():
    ...
    This tests the actual pattern from line 74: f'.*{history_source}(\\.tile.*)?.nc'
    """
    history_source = "atmos_daily"
