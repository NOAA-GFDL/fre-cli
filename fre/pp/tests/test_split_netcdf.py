...
cases = {
    "ts": {"dir": "atmos_daily.tile3", "nc": "00010101.atmos_daily.tile3.nc", "cdl": "00010101.atmos_daily.tile3.cdl"},
    "static": {"dir": "ocean_static", "nc": "00010101.ocean_static.nc", "cdl": "00010101.ocean_static.cdl"},
}
...
@pytest.mark.parametrize(
    "workdir,infile,outfiledir,varlist",
    [
        pytest.param(casedirs[0], cases["ts"]["nc"], "new_all_ts_varlist", "all", id="ts_all"),
        pytest.param(casedirs[0], cases["ts"]["nc"], "new_some_ts_varlist", ",".join(some_ts_varlist), id="ts_some"),
        pytest.param(casedirs[0], cases["ts"]["nc"], "new_none_ts_varlist", ",".join(none_ts_varlist), id="none"),
        pytest.param(casedirs[1], cases["static"]["nc"], "new_all_static_varlist", "all", id="static_all"),
        pytest.param(
            casedirs[1],
            cases["static"]["nc"],
...
def test_split_file_run(workdir, infile, outfiledir, varlist):
    ...
    split_netcdf_args = ["pp", "split-netcdf", "--file", infile, "--outputdir", outfiledir, "--variables", varlist]
...
def test_split_file_data(workdir, newdir, origdir):
    ...
    for sf in split_files:
        nccmp_cmd = ["nccmp", "-d", "--force", osp.join(origdir, sf), osp.join(newdir, sf)]
...
def test_split_file_metadata(workdir, newdir, origdir):
    ...
    all_files_equal = True
    for sf in split_files:
        nccmp_cmd = ["nccmp", "-mg", "--force", osp.join(origdir, sf), osp.join(newdir, sf)]
        sp = subprocess.run(nccmp_cmd)
        if sp.returncode != 0:
            print(" ".join(nccmp_cmd))
            all_files_equal = False
...
# test parsing yaml
@pytest.mark.parametrize(
    "component,compdir,varlist,hist_source",
    [
        ("atmos", "all_ts_varlist", "all", "none"),
        ("atmos", "some_ts_varlist", some_ts_varlist, "none"),
        pytest.param("mantle", "none_ts_varlist", "", "none", marks=pytest.mark.xfail),
        pytest.param("atmos", "all_ts_varlist", "all", "atmos_diurnal", marks=pytest.mark.xfail),
    ],
)
