...
def experiment_check(mainyaml_dir, experiment, loaded_yaml):
    ...
    for i in loaded_yaml.get("experiments"):
        ...
        if gridyaml is None:
            fre_logger.warning(
                "WARNING! no grid yaml specified! moving on, but I hope you put this info in your CMOR yaml instead"
            )
        else:
        ...
        if ppsettingsyaml is None:
            raise ValueError(
                f"could not find a path pointing to pp-settings for cmor-yamler and experiment name = {experiment}"
            )
...
class CMORYaml:
    ...
    def combine_experiment(self, yaml_content, loaded_yaml):
        ...
        """
        # Experiment Check
        cmory_path, ppsettingsy_path, gridsy_path = experiment_check(self.mainyaml_dir, self.name, loaded_yaml)
    ...
    def merge_cmor_yaml(self, cmor_list, loaded_yaml):
        ...
        result.update(yaml.load(yml_cmor, Loader=yaml.Loader))
    ...
    def clean_yaml(self, yml_dict):
        ...
        keys_clean = [
            "name",
            "platform",
            "target",  # these are needed to create the final parsed dictionary fed to cmor
            "fre_properties",
            "directories",
            "experiments",
            "build",
