...
@click.version_option(package_name="fre-cli", version=version)
...
@click.group(
    ...
    help=click.style(
        "'fre' is the main CLI click group. It houses the other tool groups as lazy subcommands.", fg="cyan"
    ),
)
@click.option(
    "-v",
...
def fre(verbose=0, quiet=False, log_file=None):
    ...
    logging.basicConfig(level=log_level, format=FORMAT, filename=log_file, encoding="utf-8")
